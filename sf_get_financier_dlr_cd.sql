create or replace function muldms.sf_get_financier_dlr_cd(p_mul_dealer_cd in varchar2,
                                        p_for_cd        in varchar2,
                                        p_outlet_cd     in varchar2)
  return varchar2 is
  lv_dlr_cd varchar2(20);
...
begin
  ...
  if lv_dlr_cd is null then
    begin
      select 'MASS'||am.mul_dealer_cd
        into lv_dlr_cd
        from am_dealer_loc am
       where am.mul_dealer_cd=p_mul_dealer_cd
         and am.for_cd=p_for_cd
  ...
  if lv_dlr_cd is null then
    begin
      select financier_delr_catg||ew.financier_dlr_cd
        into lv_dlr_cd
        from dmsd_ew_fin ew
